<template>
	<view class="login-form-container full uni-padding-wrap">
		<uni-forms ref="form" v-model="formData" class="login-form full" label-width=0>
			...
			<template v-if="formData.loginType === 'account_login'" name="password" >
				<uni-forms-item name="userName">
					<view class="login-form-item wfull">
						...
						<input v-model="formData.userName" type="text" placeholder="请输入用户名" class="uni-flex-item" @input="binddata('userName', $event.detail.value, 'form')"/>
					</view>
				</uni-forms-item>
			...
			<template v-else>
				<uni-forms-item name="phone">
					<view class="login-form-item wfull">
						...
						<input v-model="formData.phone" type="number" placeholder="请输入手机号" class="uni-flex-item" @input="binddata('phone', $event.detail.value, 'form')"/>
					</view>
				</uni-forms-item>
				<uni-forms-item name="verificationCode">
					<view class="login-form-item wfull">
						<input v-model="formData.verificationCode" type="number" placeholder="请输入验证码" class="uni-flex-item" @input="binddata('verificationCode', $event.detail.value, 'form')"/>
			...
			<!-- 其他	-->
			<button type="primary" @click="submitForm(loginEvt)" class="wfull mgt50">登 录 必填校验</button>
			<button type="primary" @click="submitForm(loginEvt)" class="wfull" :disabled="submitIsDisabled">登 录禁用</button>
...
export default {
  ...
  data() {
    return {
      showPwd: false,
      formData: {
				// account_login账号密码  phone_login验证码
  ...
  methods: {
    ...
    async loginEvt() {
      ...
      if (result.status === 200) {
        if (this.formData.rememberPwd) {
          // 30天内免密登录
          this.$uniTools.setStorage('token', result.data.token, 3600 * 24 * 30);
    ...
    sendVerificationCode() {
      if (this.verificationCodeData.codeBtnEnable) return;
      this.clearIntervalEvt();
