		...
		<template v-else>
			...
			<template v-if="loadMoreData.otherStatus === 'error'">
				<view class="load-more">
					...
					<view @click="$emit('requestErrorReflash')" :style="{color: loadMoreData.color}" class="wfull tc load-more-error">
						出错啦，点击我
			...
			<template v-else-if="loadMoreData.otherStatus === 'empty'">
				...
				<view class="load-more">
					...
					<text :style="{color: loadMoreData.color}" class="dinlineb wfull tc load-more-empty">{{ emptyText }}</text>
				</view>
			</template>
		</template>
	</view>
