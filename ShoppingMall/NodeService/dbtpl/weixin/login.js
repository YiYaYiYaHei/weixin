...
module.exports = app => {
      ...
      if (response && (response.statusCode === 200)) {
        ...
        if(data.session_key) {
          ...
          if (find_res.status === 200) {
            ...
            const token = common.encryptedToken(getTokenStr(find_res.data[0].uId, createdTime));
            let gender = '';
  ...
  app.post('/wx/refreshToken', async (req, res) => {
    ...
    if (find_res.status === 200) {
      ...
      const token = common.encryptedToken(getTokenStr(find_res.data[0].uId, loginTime));
      // 返回参数
      const resData = {
        status: 200,
        message: '刷新token成功',
